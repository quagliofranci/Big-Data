# Creates pseudo distributed hadoop 3.3.6 - CPU AMD64 
#
...
# install dev tools
RUN apt-get update
RUN apt-get install -y curl tar sudo openssh-server rsync openjdk-8-jdk-headless vim net-tools
...
# # hadoop
RUN wget https://downloads.apache.org/hadoop/common/hadoop-3.3.6/hadoop-3.3.6.tar.gz ; tar -zxf hadoop-3.3.6.tar.gz -C /usr/local/ ; rm hadoop-3.3.6.tar.gz
