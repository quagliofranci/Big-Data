...
\begin{itemize}
    \item \textbf{namenode}: nodo master per HDFS e gestione del filesystem distribuito
    \item \textbf{datanode}: nodo worker per lo storage dei dati
    \item \textbf{resourcemanager}: gestore delle risorse YARN
    \item \textbf{spark-master}: nodo master per l'esecuzione di job Spark
\end{itemize}

\subsection{Volumi e Percorsi}

Il volume \texttt{hddata} viene montato nel container come \texttt{/submit}, contenendo:
\begin{itemize}
...
# Accedere al container namenode
...
# Nel container namenode
mkdir -p build_spark
javac -encoding UTF-8 -cp "/submit/spark_libs/*" \
...
SPARK_CP=$(echo /submit/spark_libs/*.jar | tr ' ' ':')

# Esecuzione
java -cp "/submit/CustomerSpark.jar:$SPARK_CP" \
    -Dspark.master=local[*] \
    spark.SparkDriver \
